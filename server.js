...
var server = http.createServer(function(request, response) {
  ...
  if (path === "/index.html") {
    ...
    //这里用readFileSync找到的是个对象，需要转化为字符串让服务器读到
    let stringHtml = fs.readFileSync("public/index.html").toString();
    const page1 = fs.readFileSync("db/page1.json");
    //让数据展示的好看一下，先转化为数组，然后遍历这个数组没每一项的值放到li里面
    const array = JSON.parse(page1);
    const result = array.map(item => `<li>${item.id}</li>`).join("");
    //这里如果直接repalce的话是生成一个新的stringHtml，所以需要再次赋值一下，把每一个li都放到ul中
    stringHtml = stringHtml.replace("{{page1}}", `<ul id='xxx'>${result}</ul>`);
